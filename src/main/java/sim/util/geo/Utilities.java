...
public class Utilities {
   ...
   * @param descending If true, values are sorted in descending order; otherwise, they are sorted in
   *        ascending order.
   ...
   * Optionally, values higher or lower than the mean can be returned based on the provided
   * direction.
   *
  ...
  public static double fromDistribution(double mean, double sd, String direction) {
    ...
    if (direction != null) {
      if ((direction.equals("left")) && (result > mean)
          || (direction.equals("right")) && (result < mean)) {
        result = mean;
      }
    }
    if (result <= 0.00) {
  ...
  /**
   * Filters a map to keep entries where the value is greater than or equal to the provided
   * threshold value.
   *
   * @param map The input map.
   ...
   */
  private static <K, V extends Comparable<Double>> Map<K, Double> filterMapByThreshold(
      Map<K, Double> map, double thresholdValue) {
  ...
  public static <K, V> HashMap<K, V> filterMapByIndex(HashMap<K, V> map, ArrayList<K> keyList) {
    Map<K, V> filteredMap =
        map.entrySet().stream().filter(entry -> keyList.contains(entry.getKey()))
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
