...
/**
 * The class provides utility methods for working with angles and coordinates. It contains static
 * methods to calculate angles between points, compute angular differences, and perform various
...
public class Angles {
   ...
   *
   * This method calculates the angle in degrees between two NodeGraph instances, where 'originNode'
   * represents the starting point, and 'destinationNode' represents the ending point. The angle is
   * measured with respect to the positive y-axis.
   *
   ...
   * represents the origin and the second set represents the destination. The angle is measured with
   * respect to the positive y-axis and returned in degrees.
   *
  ...
  /**
   * Calculates the angular difference between two angles, considering their orientation.
   *
   * This method computes the absolute angular difference between two angles, taking into account
   * their orientations in degrees. The result is the smallest angular difference between the two
   * angles, considering both positive and negative angular directions.
   *
   ...
   *
   * This method determines if an angle, given the angle between the origin and the destination node
   * (angleOD), the angle between the origin and a possible intermediate node (angleON), and the
   * amplitude of the cone (cone), lies within the specified cone.
   *
   ...
   * @return {@code true} if the angle between the origin and the intermediate node is within the
   *         specified cone; {@code false} otherwise.
   */
   ...
   *
   * Given a NodeGraph, a distance from it, and a desired angle formed with the y axis, it returns
   * the coordinates of the resulting location.
   *
   ...
   *
   * Given two NodeGraph instances and half of the amplitude of a desired angle (either positive or
   * negative, e.g. + 35.0° or - 35.0° --> 70°), this method computes the coordinate of a point
   * that, when connected with the first node, forms one of the lines of the view cone.
   *
   ...
   */
  private static Coordinate angleViewField(NodeGraph node, NodeGraph otherNode,
      double desiredAngle) {
    ...
    final Coordinate coord =
        getCoordAngle(node, GraphUtils.nodesDistance(node, otherNode), resultingAngle);
    return coord;
  }

  /**
   * Computes the view field geometry between two nodes based on a specified field of view angle.
   * The view field represents the region visible from the first node to the second node within the
   * given field of view angle.
   *
