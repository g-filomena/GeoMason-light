...
/**
 * `VectorLayer` is an extension of the `Layer` class that offers advanced geometric, selection, and
 * filtering capabilities for managing and working with spatial data.
 */
public class VectorLayer extends Layer {
  ...
  /**
   * Creates a VectorLayer from a List of MasonGeometry objects contained in an List of
   * MasonGeometries.
   *
   * @param masonGeometries A List containing MasonGeometry objects to populate the VectorLayer
   *        with.
   */
  ...
  /**
   * Adds a MasonGeometry to the VectorLayer and updates the layer's spatial index. This method adds
  ...
  /**
   * Searches the VectorLayer for the first geometry with attribute equals to name that has the
   * given value.
   *
   ...
   *
   * This method assigns a unique identifier to each MasonGeometry object in the geometries list
   * based on the specified attribute name. The attribute value for each MasonGeometry is used as
  ...
  /**
   * Retrieves a List of IDs from the MasonGeometry objects in the geometries list. Each ID is
   * obtained from the user data of the MasonGeometry object, which is assumed to be stored as an
   * Integer. This method iterates through all MasonGeometry objects in the geometriesList and
   * collects their IDs into a new list.
   *
  ...
  /**
   * Retrieves a list of MasonGeometry objects corresponding to the provided set of IDs.
   *
  ...
  /**
   * Retrieves a List of integer values from a specified attribute column in this VectorLayer. This
   * function iterates through the geometries in the VectorLayer and extracts integer values from
   * the specified attribute column for each geometry. It returns a List of these integer values.
   *
   ...
   */
  public void setGeometryLocation(MasonGeometry masonGeometry,
      CoordinateSequenceFilter coordsFilter) {
  ...
  /**
   * Computes the convex hull of the geometries within this VectorLayer. This function calculates
   * the convex hull, which is the smallest convex polygon that encloses all the geometries in this
   * VectorLayer.
   */
  ...
  /**
   * Compute the union of the VectorLayer geometries. The resulting Geometry is the outside points
  ...
  /**
   * Queries the VectorLayer based on the provided Envelope and returns a List of MasonGeometry
   * objects that intersect with the envelope.
   *
   ...
   * @return A List containing the geometries that fall within the specified distance range from the
   *         input geometry.
   */
  public List<MasonGeometry> featuresBetweenLimits(Geometry inputGeometry, double lowerLimit,
      double upperLimit) {
  ...
  /**
   * Returns all the geometries in the VectorLayer that are contained within a certain radius from a
   * given Geometry.
   *
   * @param inputGeometry The reference geometry for containment evaluation.
   * @param radius The maximum distance from the input geometry within which other geometries are
   *        considered.
   * @return A List containing the geometries that are within the specified radius of the input
   *         geometry.
   */
  ...
  /**
   * Computes the intersection of geometries between this VectorLayer and another VectorLayer. This
   * function takes another VectorLayer as input and checks for intersections between its geometries
   * and the geometries in this VectorLayer. It returns a List of intersecting geometries if
   * 'inclusive' is true, or a list of non-intersecting geometries if 'inclusive' is false.
   *
   * @param otherLayer The VectorLayer to intersect with this VectorLayer.
   * @param inclusive If true, returns intersecting geometries; if false, returns non-intersecting
   *        geometries.
  ...
  /**
   * Checks whether the input MasonGeometry is covered by other geometries in the spatial index of
   * the VectorLayer.
   *
   * @param inputMasonGeometry The MasonGeometry object to check for coverage.
   * @return true if the inputMasonGeometry is covered by other geometries in the spatial index,
   *         false otherwise.
   */
  public boolean isCovered(MasonGeometry inputMasonGeometry) {
    ...
    if (inputMasonGeometry.preparedGeometry == null) {
      inputMasonGeometry.preparedGeometry =
          PreparedGeometryFactory.prepare(inputMasonGeometry.getGeometry());
    }
  ...
  /**
   * Filters a List of geometries based on a specified String attribute's value. This method selects
   * geometries that either match or do not match the provided attribute value based on the 'equal'
   ...
   *        If false, filter geometries where the specified attribute does not equal the given
   *        value.
   * @return A List containing the filtered MasonGeometry features. The List may be empty if no
   *         matching geometries are found.
   */
  public List<MasonGeometry> filterFeatures(String attributeName, String attributeValue,
      boolean equal) {
  ...
  /**
   * Filters a List of geometries based on a List of String values. This function filters the
   * geometries in this VectorLayer based on the provided string attribute and a List of string
   * values. If 'equal' is true, it keeps features with attribute values that match those in the
   * list; if 'equal' is false, it keeps features with attribute values that do not match the list.
   * The selected features are added to a new list and returned.
   *
   ...
   * @param equal If true, keeps features with attribute values matching the list; if false, keeps
   *        others.
   * @return A List of MasonGeometry features after filtering.
   */
  public List<MasonGeometry> filterFeatures(String attributeName, List<String> listValues,
      boolean equal) {
    final List<MasonGeometry> filteredFeatures = new ArrayList<>();
    for (final MasonGeometry masonGeometry : geometriesList) {
      final String attribute = masonGeometry.getStringAttribute(attributeName);
  ...
  /**
   * Filters a List of geometries in the VectorLayer based on a specified integer attribute's value.
   * This method selects geometries that either match or do not match the provided integer attribute
   * value based on the 'equal' parameter. Matching geometries are included in the resulting List.
   *
   * @param attributeName The name of the integer attribute to filter by. This should be a valid
   *        attribute name present in the MasonGeometry features.
   * @param attributeValue The integer value to compare against for filtering. Geometries with the
   *        specified integer attribute equal to this value will be included in the result.
   * @param equal If true, filter geometries where the specified integer attribute equals the given
   *        value. If false, filter geometries where the specified integer attribute does not equal
   *        the given value.
   * @return A List containing the filtered MasonGeometry features. The List may be empty if no
   *         matching geometries are found.
   */
  public List<MasonGeometry> filterFeatures(String attributeName, int attributeValue,
      boolean equal) {
  ...
  /**
   * Selects and creates a new VectorLayer containing features based on a specified integer
   * attribute and values. This function filters the features in this VectorLayer based on the
   * provided integer attribute and a List of integer values. If 'equal' is true, it keeps
   * geometries with attribute values that match those in the list; if 'equal' is false, it keeps
   * geometries with attribute values that do not match the list. The selected geometries are added
   * to a new VectorLayer, which is returned.
   *
   ...
   * @return A List of MasonGeometry objects in the VectorLayer that are covered by the input
   *         MasonGeometry.
   */
  public final List<MasonGeometry> coveredFeatures(MasonGeometry inputMasonGeometry) {
    List<MasonGeometry> coveredFeatures = new ArrayList<>();
    if (inputMasonGeometry.preparedGeometry == null) {
      inputMasonGeometry.preparedGeometry =
          PreparedGeometryFactory.prepare(inputMasonGeometry.getGeometry());
    }
  ...
  /**
   * Retrieves all the geometries in the VectorLayer that are fully contained within the boundaries
   * of a given Geometry.
   *
   * @param inputGeometry The reference geometry defining the containment boundary.
   * @return A List containing the geometries that are entirely contained within the specified input
   *         geometry.
   */
  ...
  /**
   * Reads a Shapefile from the specified input directory and populates the provided VectorLayer
   * with geometries. The Shapefile consists of two files: a .shp file containing geometry
   * information and a .dbf file containing attribute data. The function reads both files and adds
   * the geometries to the VectorLayer.
   *
   ...
   */
  public static void readShapefile(URL shpURL, URL dbfURL, VectorLayer vectorLayer)
      throws Exception {
  ...
  public void updateTransform(DrawInfo2D info) {
    ...
    if (!info.equals(myInfo)) {
      ...
      Point2D p1 =
          GeometryUtilities.screenToWorldPointTransform(worldToScreen, info.clip.x, info.clip.y);
      Point2D p2 = GeometryUtilities.screenToWorldPointTransform(worldToScreen,
          info.clip.x + info.clip.width, info.clip.y + info.clip.height);
