...
public class GeomVectorField extends GeomField {
	...
	/**
	 * Redundant container of MasonGeometry used to quickly rebuild spatial index
	 * and save some overhead with regards to returning all MasonGeometry.
	 *
	 ...
	 * significant chunks of the spatial index may have to be rebuilt depending on
	 * the removed geometry. Therefore the spatial index is not updated to reflect
...
//        spatialIndex.remove(g.getGeometry().getEnvelopeInternal(), g);
	...
	/**
	 * Compute the union of the field's geometries. The resulting Geometry is the
	 * outside points of the field's geometries. Call this method only once.
	 */
...
//              List<?> gList = spatialIndex.queryAll();
	 ...
	 * determines the distance between the closest points of two geometries. Do not
	 * modify the returned Bag.
	 */
	...
	public void updateSpatialIndex() {
		if (needToRebuildIndex) {
			...
			for (int i = 0; i < geometries.size(); i++) {
				spatialIndex.insert(((MasonGeometry) geometries.get(i)).geometry.getEnvelopeInternal(),
						geometries.get(i));
			}
