...
public class Islands {
  ...
  public Set<EdgeGraph> mergeConnectedIslands(Set<EdgeGraph> edges) {
    ...
    while (islands.size() > 1) {
      ...
      if (connectingEdge != null) {
        ...
        Map.Entry<Pair<NodeGraph, NodeGraph>, Set<NodeGraph>> closestPairWithIsland =
            findClosestPairAcrossAllIslands();
  ...
  /**
   * Finds the closest pair of nodes across all islands and returns the pair along with the island
   * of the second node.
   *
   * @return a map entry containing the closest pair of nodes and the island of the second node
   */
  private Map.Entry<Pair<NodeGraph, NodeGraph>, Set<NodeGraph>> findClosestPairAcrossAllIslands() {
    return islands.parallelStream().flatMap(island -> islands.parallelStream()
        .filter(otherIsland -> island != otherIsland)
        ...
        .min(Comparator.comparingDouble(entry -> GraphUtils
            .nodesDistance(entry.getKey().getValue0(), entry.getKey().getValue1())))
        .orElse(null);
  }
  ...
  private static EdgeGraph findConnectingBridge(List<Set<NodeGraph>> islands, Graph graph) {
    return islands.stream().flatMap(island -> islands.stream()
        .filter(otherIsland -> otherIsland != island)
