...
/**
 * A node of a planar graph that extends the `Node` class (JTS), with additional and more
 * straightforward functions. This class is one of the two components, along with `EdgeGraph`, of
 * the graphs belonging to the `Graph` class.
 */
public class NodeGraph extends Node {

  /**
   * Constructs a new NodeGraph instance with the specified coordinate. This node represents a point
   * in the planar graph.
   *
  ...
  /**
   * Sets the centrality score for this node. The centrality score reflects the node's importance
   * within the graph based on its connections.
   *
  ...
  /**
   * Retrieves the centrality score of this node. The centrality score indicates how important or
   * central a node is within the graph.
   *
  ...
  /**
   * Sets lists useful for identifying the neighboring components of this node. This method
   * initializes the edge and adjacent node lists.
   */
  ...
  /**
   * Identifies and sets the list of all the edges (non-directed) that depart from this node. This
   * method initializes the 'edges' list with non-duplicated edges, representing the connections
   * from this node to its adjacent nodes.
   */
  ...
  /**
   * Returns a List of integers representing all the adjacent regions to this node, if any. This
   * method is relevant for nodes that act as gateways between different regions. It returns null
   * for non-gateway nodes.
   *
   * @return a List of integers representing adjacent regions, or null if this node is not a
   *         gateway.
   */
  ...
  /**
   * Returns a List of directed edges that depart from this node (out-going edges). These edges
   * represent the connections from this node to other nodes in the graph.
   *
  ...
  /**
   * Returns a node in the dual representation of the graph, which represents a segment departing
   * from this node (a node in a dual graph represents an actual street segment). When the node for
   * which the dual-node is desired is the originNode of a desired route, the segment closest to the
   * destination is chosen (~ towards it). When the node for which the dual-node is desired is the
   * destinationNode, the segment closest to the origin is chosen.
   *
   ...
   * @param previousJunction Node to avoid choosing dual nodes representing segments that should not
   *        be traversed.
   * @return The selected dual node representing a street segment.
   */
  public NodeGraph getDualNode(NodeGraph originNode, NodeGraph destinationNode,
  ...
  /**
   * Returns a map of nodes in the dual representation of the graph, which represent segments
   * departing from this node (a node in a dual graph represents an actual street segment). When the
   * node for which the dual-nodes are desired is the originNode of a desired route, the map is
   * sorted on the basis of the distance from the dual nodes (segments' centroids) to the
   * destination node (~ towards it). When the node for which the dual-nodes are desired is the
   * destinationNode, the map is sorted on the basis of the distance from the dual nodes (segments'
   * centroids) to the origin node.
   *
   * This function is preferable to the one above as it allows considering multiple segments as
   * possible "departures" for route planning algorithms. When computing paths within subgraphs,
   * some specific segments may be indeed unreachable.
   *
   * @param originNode The origin node of the desired route.
   * @param destinationNode The destination node of the desired route.
   * @param regionBasedNavigation Indicates if the agent is navigating through regions.
  ...
  /**
   * Checks whether the previous junction is the same as the one this dualNode leads to.
   *
   * @param previousJunction The previous junction to compare.
   * @param dualNode The dual node to check against.
   * @return True if the previous junction matches either the "from" or "to" node of the primal edge
   *         connected to the dual node.
   */
