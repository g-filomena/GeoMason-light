...
/**
 * The portrayal for MasonGeometry objects. The class draws the JTS geometry object (currently, we
 * can draw Point, LineString, Polygon, MultiLineString and MultiPolygon objects), and sets up the
 * inspectors for the MasonGeometry object. The inspector is TabbedInspector with at most three
 * tabs: the first tab shows various information about the JTS geometry, the second tab shows the
 * associated attribute information, and the third tab shows information about the MasonGeometry
 * userData field, which can be any Java object.
 */
public class GeomPortrayal extends SimplePortrayal2D {
  ...
  public Inspector getInspector(LocationWrapper wrapper, GUIState state) {
    ...
    if (o instanceof MasonGeometry) {
      ...
      if (gw.hasAttributes()) {
        if (!gw.hasHiddenAttributes()) { // only add attributes tag if JTS geometry has attributes
  ...
  /**
   * Draw a JTS geometry object. The JTS geometries are converted to Java general path objects,
   * which are then drawn using the native Graphics2D methods.
   */
  @Override
    ...
    if ((gm.isMovable) || (gm.shape == null) || (!gm.transform.equals(gInfo.transform))) {
      ...
      if (geometry instanceof Point) {
        ...
        Ellipse2D.Double ellipse = new Ellipse2D.Double(point.getX() - offset,
            point.getY() - offset, 3 * scale, 3 * scale);

        GeneralPath path =
            (GeneralPath) (new GeneralPath(ellipse).createTransformedShape(gInfo.transform));
        gm.shape = path;
   ...
   *
   * Polygons have two sets of coordinates; one for the outer ring, and optionally another for
   * internal ring coordinates. Draw the outer ring first, and then draw each internal ring, if they
   * exist.
   */
  GeneralPath drawPolygon(Polygon polygon, GeomInfo2D info, boolean fill) {
    ...
    for (int i = 0; i < polygon.getNumInteriorRing(); i++) { // fill for internal rings will always
                                                             // be false as they
                                                             // are literally
  ...
  /**
   * Helper function to draw a JTS geometry object. The coordinates of the JTS geometry are
   * converted to a native Java GeneralPath which is used to draw the object.
   */
  GeneralPath drawGeometry(Geometry geom, GeomInfo2D info, boolean fill) {
    Coordinate coords[] = geom.getCoordinates();
    GeneralPath path = new GeneralPath(Path2D.WIND_NON_ZERO, coords.length);
