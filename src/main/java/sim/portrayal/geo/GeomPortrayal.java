...
public class GeomPortrayal extends SimplePortrayal2D {
	...
	/**
	 * Use our custom Inspector. We create a TabbedInspector for each object that
	 * allows inspection of the JTS geometry, attribute information, and the
	 * MasonGeometry userData field.
	 */
	@Override
	...
	/**
	 * Draw a JTS geometry object. The JTS geometries are converted to Java general
	 * path objects, which are then drawn using the native Graphics2D methods.
	 */
	@Override
	...
	GeneralPath drawPolygon(Polygon polygon, GeomInfo2D info, boolean fill) {
		...
		for (int i = 0; i < polygon.getNumInteriorRing(); i++) { // fill for internal rings will always be false as they
																	// are literally
	 ...
	 * geometry are converted to a native Java GeneralPath which is used to draw the
	 * object.
	 */
